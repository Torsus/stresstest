        ...
        private void button2_Click(object sender, EventArgs e)
        {
          ...
          //  command = new SqlCommand(Sql, cnn);
           ...
           // int personnummerindex;
            ...
            {
                Sql = "INSERT INTO " + tabell2 + " (Patient,type,[Analysis number],[Analysis Template],Address,[Answered Date],Question,Answer,Signature,[Submitter Name],Submitter,Type,Urgent,[Arrived date],[Invoice Date],Indication,Research,[Saved material],Canister,Box,Number,Remark,[Orderer Internal],Orderer,[Accountable Internal],Accountable,Result,Quality,Diagnosis,McKusick,Answered,Price,[Invoice Nr],Doctor,Metaphases,Locked,Change,[Result Internal],Prenatal,[Result Internal 2],Family,[Care Date],[Specimen Type])";
                Sql += "VALUES(@Patient,@type,@Analysnumber,@Analysis_template,@Address,@Answered_Date,@Question,@Answer,@Signature,@Submitter_Name,@Submitter,@Type,@Urgent,@Arrived_date,@Invoice_Date,@Indication,@Research,@Saved_material,@Canister,@Box,@Number,@Remark,@Orderer_Internal,@Orderer,@Accountable_Internal,@Accountable,@Result,@Quality,@Diagnosis,@McKusick,@Answered,@Price,@Invoice_Nr,@Doctor,@Metaphases,@Locked,@Change,@Result_Internal,@Prenatal,@Result_Internal_2,@Family,@Care_Date,@Specimen_Type)";
                command = new SqlCommand(Sql, Datacontainer.cnn);
                command.Parameters.Add(new SqlParameter("@patient", Datacontainer.personnummerindex));
